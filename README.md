...
The patches preserve the upstream CLI's ordinary `Start` behavior, while adding
...
the process through `log.Fatalf`. They also emit fixed, provider-free TUN
stage markers strictly for the Crew wrapper's redacted failure classification.
